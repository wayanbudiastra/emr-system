...
async function fetchRad(search: string, page: number) {
  const q = new URLSearchParams({ limit: "10", kategori: "RADIOLOGI", page: String(page) });
...
function RadForm({ open, onClose, item, onSuccess }: { open: boolean; onClose: () => void; item?: Item | null; onSuccess: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const body = item
        ...
        : { kode: form.kode.toUpperCase(), nama: form.nama, kategori: "RADIOLOGI", tarif: Number(form.tarif), tarifBPJS: form.tarifBPJS ? Number(form.tarifBPJS) : undefined, satuanWaktu: form.satuanWaktu || undefined };
      const r = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (!r.ok) { const e = await r.json(); throw new Error(e.error?.formErrors?.[0] ?? e.error ?? "Error"); }
    },
    onSuccess: () => { toast.success(item ? "Data diperbarui" : "Item radiologi dibuat"); onSuccess(); onClose(); },
  ...
  return (
    <Dialog open={open} onOpenChange={o => !o && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader><DialogTitle>{item ? "Edit Item Radiologi" : "Tambah Item Radiologi"}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {!item && (
              <div className="col-span-2 space-y-1">
                <Label>Kode *</Label>
                <Input value={form.kode} onChange={e => setForm(p => ({ ...p, kode: e.target.value }))} placeholder="R001" />
              </div>
            )}
            ...
            <div className="col-span-2 space-y-1"><Label>Waktu Pengerjaan</Label><Input value={form.satuanWaktu} onChange={e => setForm(p => ({ ...p, satuanWaktu: e.target.value }))} placeholder="30 menit, 2 jam" /></div>
          </div>
        </div>
        <DialogFooter>
...
export default function RadiologiPage() {
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: ["radiologi"] });

  useEffect(() => { setPage(1); }, [search]);

  const { data, isLoading } = useQuery({ queryKey: ["radiologi", search, page], queryFn: () => fetchRad(search, page) });
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="relative flex-1 max-w-xs">
          ...
          <Input placeholder="Cari item radiologi..." className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? Array.from({ length: 5 }).map((_, i) => (
              ...
              <TableRow><TableCell colSpan={5} className="text-center py-12 text-muted-foreground">Tidak ada item radiologi</TableCell></TableRow>
            ) : items.map(item => (
      ...
      <RadForm open={createOpen} onClose={() => setCreateOpen(false)} onSuccess={refresh} />
      <RadForm open={Boolean(editItem)} onClose={() => setEditItem(null)} item={editItem} onSuccess={refresh} />
    </div>
  );
}
